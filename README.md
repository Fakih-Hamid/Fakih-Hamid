## Focus
• Cybersecurity Research<br>
• Application Development<br>
• Penetration Testing


## Current Work
I am a Master’s student and Research Assistant at the Cyber-Resilience Lab, NAIST, where I am involved in research on privacy-preserving cybersecurity education frameworks. In my free time, I focus on hands-on security practice, including penetration testing, security experimentation, and building applications. 

## Selected Projects
- **Project Aegis**  
  Autonomous fuzzing environment with guided coverage and policy-based sandboxing.
- **Security Tool (C++)**  
  Windows-focused system integrity and anti-cheat demonstrator using modern C++.
- **Industrial Security Modeling**  
  Research-oriented tool for modeling ICS/OT dependencies and cybersecurity impact analysis.


## 🌐 Portfolio
<p align="center">
  <a href="https://fakih-hamid.github.io" target="_blank">
    <img src="https://img.shields.io/badge/View%20Portfolio-fakih--hamid.github.io-orange?style=for-the-badge" />
  </a>
</p>

## 📫 Contact
- Email: fakih-hamid@proton.me
